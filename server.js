...
const commands = {
	...
	'reboot': (msg) => {
		if (msg.author.id == tokens.adminID) process.exit(0); //Requires a node module like Forever to work.
	}
};

client.on('ready', () => {
	console.log(`Bot is online. Username: ${client.user.username}. ID: ${client.user.id}. Tag: ${client.user.tag}. Created: ${client.user.createdAt}. Administrator: ${tokens.adminID}.`);
});
